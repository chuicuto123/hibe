...
if getgenv().ConfigsKaitun == nil then
    getgenv().ConfigsKaitun = {
        ...
        AutoPlant = {
            ...
            UsePlantQuota  = true,
            -- Ten cay map 1 (Main) + map 2 (FallHarvest = "Maple <ten>"). Key khong ton tai o world dang
            -- choi = thua, vo hai.
            PlantQuota = {
                ["Carrot"] = 10,
                ["Strawberry"] = 4,
                ...
                ["Maple Coconut"] = 50,
                ["Maple Mango"] = 50,
                ["Maple Dragon Fruit"] = 50,
                ["Maple Acorn"] = 50,
                ["Maple Cherry"] = 50,
                ["Maple Sunflower"] = 50,
                ["Maple Venus Fly Trap"] = 50,
                ["Maple Pomegranate"] = 50,
                ["Maple Poison Apple"] = 50,
                ["Maple Venom Spitter"] = 50,
                ["Atlantic Giant Pumpkin"] = 50,
                ["Conifer Cone"] = 50,
                ["Amber Cranberry"] = 50,
                ["Romanesco"] = 50,
            },
        ...
        AutoBuyGear = {
            ...
            PriorityList = {
                "Super Syrup Sprinkler",   -- map 2 (FallHarvest) == Super Sprinkler
                "Super Sprinkler",
                "Legendary Sprinkler",
                "Rare Sprinkler",
                "Uncommon Sprinkler",
                "Syrup Sprinkler",         -- map 2 == Common Sprinkler
                "Common Sprinkler",
            },
            ExcludeList = {
                "Jump Mushroom",
...
do
    ...
    -- GO TO EVENT (chồng yêu cầu): true = đang ở MAP 1 thì tự gọi remote đi qua MAP EVENT (map 2) để farm.
    --   ["Go To Event"] = true          -> tự dò world event trong menu du hành
    --   ["Go To Event"] = "FallHarvest" -> ép đi đúng world này
    -- false / không khai = TẮT (script chạy map nào farm map đó, không tự đi đâu).
    if type(CFG["Go To Event"]) == "boolean" then
        ensure("GoToEvent").Enabled = CFG["Go To Event"]
    elseif type(CFG["Go To Event"]) == "string" and CFG["Go To Event"] ~= "" then
        ensure("GoToEvent").Enabled = true
        ensure("GoToEvent").Target = CFG["Go To Event"]
    end
...
-- (DA GO theo yeu cau chong: BlockWorldTravel / AntiTeleport.
--  Script KHONG chan teleport nua - chay o map nao thi farm theo logic map do.
--  Rieng bo loc blast VAN giu chan prompt "Travel to ..." o isDangerBlastPrompt: do la chong bot
--  BAN NHAM cong dich chuyen luc claim seed, khong phai chan chong tu bam.)
-- ===== GO TO EVENT (chong yeu cau): dang o MAP 1 -> tu goi remote di qua MAP EVENT de farm =====
-- Mac dinh TAT. Bat bang 1 dong trong config:  ["Go To Event"] = true
CFG.GoToEvent = CFG.GoToEvent or {}
setDefault(CFG.GoToEvent, "Enabled", false)
setDefault(CFG.GoToEvent, "Target", "")        -- "" = TU DO world event trong menu du hanh; hoac ep "FallHarvest"
setDefault(CFG.GoToEvent, "StartDelay", 12)    -- doi game load xong roi moi di
setDefault(CFG.GoToEvent, "RetryDelay", 30)    -- server tu choi / chua du dieu kien -> thu lai sau N giay
setDefault(CFG.GoToEvent, "MaxTries", 5)       -- so lan FIRE toi da (0 = khong gioi han)
setDefault(CFG.GoToEvent, "WaitTutorial", true)-- true = doi xong tutorial moi di (game chan nhieu thao tac khi con tutorial)
...
-- Quota mac dinh cho CA HAI world: ten map 1 (Main) + ten map 2 (FallHarvest = "Maple <ten>" + cay fall
-- rieng, XAC NHAN SeedData.lua Worlds={"FallHarvest"}). Ten khong ton tai o world dang choi = key thua,
-- khong anh huong (AutoPlant chi tra quota theo ten cay THUC su co trong tui).
...
-- Gom CA hai world: map 1 (Main) = Super/Legendary/Rare/Uncommon/Common Sprinkler; map 2 (FallHarvest)
-- = Super Syrup Sprinkler / Syrup Sprinkler (XAC NHAN GearShopData.lua: ItemType "Super Sprinkler" /
-- "Common Sprinkler", cung chi so SprinklerData: Syrup == Common, Super Syrup == Super). Ten khong ton tai
-- o world hien tai chi la key thua trong bang uu tien -> vo hai.
...
-- ===== TOC DO NHAT DROP THUONG (chong bao "nhat cham") - xem chu thich o Runtime.ClusterTouchDrops =====
-- CUM: dung 1 cho -> touch + blast MOI drop trong ban kinh nay -> 1 lan di chuyen nhat ca cum.
setDefault(CFG.AutoCollectDrops, "ClusterClaim", true)
setDefault(CFG.AutoCollectDrops, "ClusterRadius", 20)          -- studs; ~= BlastPromptRadius, dung to qua keo lag blast
setDefault(CFG.AutoCollectDrops, "ClusterBlastPrompts", true)
setDefault(CFG.AutoCollectDrops, "ClusterBlastInterval", 0.15) -- nhip toi da cua blast trong 1 cum (chong ban moi item)
-- Da dung trong ban kinh nay thi KHONG di chuyen nua (0 = tat, luon di chuyen nhu cu).
-- Mac dinh 6 = DUNG nguong TweenMoveTo da tu bo qua tu truoc (dong "if dist <= 6 then return true"),
-- nen KHONG doi hanh vi claim, chi bo phan tinh PickRouteButton + TeleportWait thua. Muon nhanh hon nua
-- thi tang dan (10-14) roi xem log co tut ti le claim khong - xa qua thi prompt ngoai tam = nhat HUT.
setDefault(CFG.AutoCollectDrops, "DropSkipMoveRadius", 6)
-- Tween RIENG cho drop thuong. false = set CFrame thang (nhanh hon nhieu: tween 22 studs/s = ~1s/item).
-- Seed event KHONG dung co nay -> van tween muot nhu cu.
setDefault(CFG.AutoCollectDrops, "DropUseTween", CFG.AutoCollectDrops.UseTweenTeleport ~= false)
...
do
    ...
    -- ===== WORLD ADAPTER (map 2 "FallHarvest") — XAC NHAN SOURCE =====
    -- ReplicatedStorage.SharedModules.Worlds: .CurrentId/.Current/.IsMain, .Current.CurrencyName,
    -- .EntryAvailableHere(entry) (Worlds.lua:302 - entry.Worlds == nil -> moi world; co list -> phai chua worldId).
    -- Map 1 = "Main" (tien "Sheckles"), map 2 = "FallHarvest" (tien "Leaves", seed ten "Maple <ten>").
    -- SeedData/GearShopData/PetData deu co truong .Worlds -> phai LOC theo world, khong thi bot doi mua
    -- do cua map kia (khong bao gio co stock).
    local worldsModule = shared and shared:FindFirstChild("Worlds")
    if worldsModule then
        local ok, res = pcall(require, worldsModule)
        if ok and type(res) == "table" then
            Runtime.Worlds = res
        else
            logw("require(Worlds) loi:", res)
        end
    else
        logw("Khong thay ReplicatedStorage.SharedModules.Worlds (coi nhu map 1 / khong gating world)")
    end

    -- Environment: biet dang o PLACE nao (placeType/worldId/isBotContainmentPlace) - XAC NHAN SOURCE
    -- Environment.lua: places = { Primary/FirstSession/NewUser/FallHarvest/FallHarvestBotUser/... },
    -- isBotContainmentPlace = true khi placeType la BotUser/FallHarvestBotUser (place NHOT BOT).
    local envModule = shared and shared:FindFirstChild("Environment")
    if envModule then
        local ok, res = pcall(require, envModule)
        if ok and type(res) == "table" then
            Runtime.Environment = res
        else
            logw("require(Environment) loi:", res)
        end
    end
end

-- Id world hien tai ("Main" | "FallHarvest"). Thieu module -> nil (bo qua moi gating).
Runtime.WorldId = (type(Runtime.Worlds) == "table") and Runtime.Worlds.CurrentId or nil

-- Ten stat tien trong leaderstats theo world (Worlds.Current.CurrencyName). Fallback "Sheckles" (map 1).
Runtime.CurrencyStatName = function()
    local w = Runtime.Worlds
    if type(w) == "table" then
        -- WalletStatName(player) xu ly ca truong hop dang o place PetHunt (doc attribute PetHuntOriginWorld).
        if type(w.WalletStatName) == "function" then
            local ok, name = pcall(w.WalletStatName, LocalPlayer)
            if ok and type(name) == "string" and name ~= "" then return name end
        end
        if type(w.Current) == "table" and type(w.Current.CurrencyName) == "string" and w.Current.CurrencyName ~= "" then
            return w.Current.CurrencyName
        end
    end
    return "Sheckles"
end

-- Entry (seed/gear/pet) co dung duoc o world nay khong? Thieu module Worlds -> tu doc entry.Worlds,
-- thieu ca worldId -> cho qua (khong chan nham).
Runtime.EntryAvailableHere = function(entry)
    if type(entry) ~= "table" then return false end
    local w = Runtime.Worlds
    if type(w) == "table" and type(w.EntryAvailableHere) == "function" then
        local ok, res = pcall(w.EntryAvailableHere, entry)
        if ok then return res == true end
    end
    local list = entry.Worlds
    if type(list) ~= "table" then return true end
    local id = Runtime.WorldId
...
-- ============================================================
-- TIEN: ten stat trong leaderstats DOI THEO WORLD (Worlds.Current.CurrencyName) - map 1 "Sheckles",
-- map 2 (FallHarvest) "Leaves". Data replica van giu key "Sheckles" o CA HAI map (verify LIVE:
-- PlayerStateClient:GetLocalReplica().Data.Sheckles) -> dung lam nguon du phong khi leaderstats chua ve.
local function getSheckles()
    local ls = LocalPlayer:FindFirstChild("leaderstats")
    if ls then
        local s = ls:FindFirstChild(Runtime.CurrencyStatName())
        if s == nil then s = ls:FindFirstChild("Sheckles") end   -- du phong: world khac ma stat chua doi ten
        if s ~= nil then
    ...
    if PlayerStateClient then
        ...
        local data = ok and type(replica) == "table" and replica.Data or nil
        if type(data) == "table" then return tonumber(data.Sheckles) end
    end
    return nil
end
...
local function buildSeedCandidates(c)
    ...
    for _, data in ipairs(SeedData) do
        -- LOC WORLD: map 2 (FallHarvest) chi ban seed co Worlds chua "FallHarvest"; seed map 1 van nam
        -- trong SeedData + co node trong StockValues.SeedShop.Items nhung stock LUON 0 -> khong loc thi
        -- vong nao cung quet thua ~40 seed vo ich.
        if type(data) == "table" and data.RestockShop and type(data.SeedName) == "string"
            and Runtime.EntryAvailableHere(data) then
...
-- MAP 2 (FallHarvest): pet rieng cua world - XAC NHAN ReplicatedStorage.SharedData.PetData
-- (Worlds={"FallHarvest"}): Dog Uncommon 50k, Turkey Rare 100k, Hedgehog Rare 140k, Squirrel Legendary
-- 1.2M, Swan Legendary 1.6M, Wolf Mythic 6.5M, Fox Mythic 10M, ShadowDragon Super 30M.
local PetRarityByName = {
    ...
    dog = "Uncommon",
    turkey = "Rare",  hedgehog = "Rare",
    squirrel = "Legendary",  swan = "Legendary",
    wolf = "Mythic",  fox = "Mythic",
    shadowdragon = "Super",
}
...
local function buildGearCandidates(c, gearState)
    ...
    for _, data in ipairs(GearShopData.Data) do
        -- LOC WORLD nhu seed: map 2 dung ban "Syrup"/"Super Syrup" (Worlds={"FallHarvest"}), ban Main
        -- (Common/Super Sprinkler, Common/Super Watering Can) khong bao gio co stock o day.
        if type(data) == "table" and not data.RobuxOnly and type(data.ItemName) == "string"
            and not excluded[data.ItemName] and Runtime.EntryAvailableHere(data) then
...
-- (DA GO: WorldTravelGuard / AntiTeleport - theo yeu cau chong. Script khong chan teleport nua.)

-- ============================================================
-- GO TO EVENT: dang o MAP 1 -> TU GOI REMOTE di qua MAP EVENT (map 2) de farm. Bat: ["Go To Event"]=true
-- XAC NHAN SOURCE (khong bia mot dong nao):
--   Networking.lua:22     Worlds.RequestTravel  = ("WorldsRequestTravel", String)   <- ban worldId
--   Networking.lua:24-25  Worlds.TravelStarted(String,String) / TravelFailed(String) <- server tra loi
--   EventWorldsTeleporterController.lua:279  nut "Teleport" trong GUI Event Worlds goi DUNG remote nay
--   Worlds.lua:225  GetTravelTargets() -> danh sach world DI DUOC; tra {} khi isBotContainmentPlace
--   Worlds.lua:249  GetPlaceId(world) = place CHINH  |  :259 GetBotPlaceId(world) = place NHOT BOT
-- DA DO THAT (bridge): fire RequestTravel("FallHarvest") tu FirstSession -> server tra
--   "Fall Harvest | Teleport" -> vao 126987765280963 = MAP 2 CHINH (botPlace=false). Chay duoc.
--
-- 3 truong hop KHONG fire (fire chi ton cong):
--   (1) DA o world dich roi
--   (2) acc dang o place NHOT BOT -> GetTravelTargets()={} + server tu lai ve GetBotPlaceId
--   (3) world dich chua co trong menu (cong event chua mo / chua du dieu kien)
-- ============================================================
Runtime.PickEventWorld = function()
    local c = CFG.GoToEvent
    local W = Runtime.Worlds
    if type(W) ~= "table" or type(W.GetTravelTargets) ~= "function" then return nil, "khong co module Worlds" end
    local ok, targets = pcall(W.GetTravelTargets)
    if not ok or type(targets) ~= "table" then return nil, "GetTravelTargets loi" end
    if #targets == 0 then return nil, "menu du hanh RONG" end

    local want = tostring(c and c.Target or "")
    for _, v in ipairs(targets) do
        if type(v) == "table" and type(v.Id) == "string" then
            if want ~= "" then
                if v.Id == want then return v.Id end
            elseif v.Id ~= "Main" then
                return v.Id            -- "" = tu do: world nao khong phai Main thi la world EVENT
            end
        end
    end
    return nil, (want ~= "" and ("menu chua co " .. want) or "menu chi co Main")
end

Runtime.SetupGoToEvent = function()
    local c = CFG.GoToEvent
    if not (c and c.Enabled == true) or Runtime.GoToEventReady then return end
    Runtime.GoToEventReady = true

    task.spawn(function()
        if not waitAlive(math.max(tonumber(c.StartDelay) or 12, 0)) then return end

        -- Nghe ket qua server tra ve -> biet CHAP NHAN hay TU CHOI kem LY DO THAT (khong doan).
        local started, failReason = false, nil
        for _, spec in ipairs({ { "TravelStarted", true }, { "TravelFailed", false } }) do
            local p = packet({ "Worlds", spec[1] })
            if p and p.OnClientEvent and type(p.OnClientEvent.Connect) == "function" then
                local conn = p.OnClientEvent:Connect(function(a, b)
                    if spec[2] then
                        started = true
                        actionLog("GoToEvent", "ACCEPTED", tostring(a) .. " " .. tostring(b or ""))
                    else
                        failReason = tostring(a)
                        actionLog("GoToEvent", "REFUSED", tostring(a))
                    end
                end)
        ...
        local maxTries = tonumber(c.MaxTries) or 5
        local tries = 0
        while isAlive() and not started do
            -- (1) DA o world dich -> xong viec, khong lam gi nua.
            local want = tostring(c.Target or "")
            if Runtime.WorldId ~= nil and ((want ~= "" and Runtime.WorldId == want)
                or (want == "" and Runtime.WorldId ~= "Main")) then
                actionLog("GoToEvent", "DONE", "da o world event: " .. tostring(Runtime.WorldId))
                return
            end

            -- (2) Acc bi NHOT BOT -> menu rong, server tu lai ve bot place -> fire vo ich.
            local env = Runtime.Environment
            if type(env) == "table" and env.isBotContainmentPlace == true then
                actionLog("GoToEvent", "BOT_PLACE",
                    ("acc o place nhot bot (%s) -> khong di duoc bang remote"):format(tostring(env.placeType)))
                return
            end

            if maxTries > 0 and tries >= maxTries then
                actionLog("GoToEvent", "GIVEUP", ("da fire %d lan ma khong qua duoc"):format(tries))
                return
            end

            -- (3) Tutorial: game chan nhieu thao tac khi workspace.InTutorial=true.
            if c.WaitTutorial ~= false and workspace:GetAttribute("InTutorial") == true then
                actionLog("GoToEvent", "WAIT", "dang trong tutorial -> cho xong roi di")
            else
                local target, why = Runtime.PickEventWorld()
                if not target then
                    actionLog("GoToEvent", "WAIT", tostring(why or "chua di duoc"))
                else
                    tries = tries + 1
                    failReason = nil
                    actionLog("GoToEvent", "GO", ("%s -> %s (lan %d)"):format(
                        tostring(Runtime.WorldId or "?"), target, tries))
                    firePacket({ "Worlds", "RequestTravel" }, target)
                    -- teleport that thi script chet theo (sang server moi) -> khong can lam gi them
                    if not waitAlive(6) then return end
                    if started then return end
                    if failReason then
                        actionLog("GoToEvent", "RETRY", "server tu choi: " .. tostring(failReason))
                    end
                end
            end

            if not waitAlive(math.max(tonumber(c.RetryDelay) or 30, 5)) then return end
        end
    end)
end
...
Runtime.KSFArm = function(cropName, mutName)
    ...
    if type(mutName) == "string" and mutName ~= "" then
        ...
        -- CAY THU HOACH 1 LAN an KHONG DU he so dot bien. XAC NHAN SOURCE (FruitValueCalc.lua):
        --   if isSingleHarvest[name] and mult > 1 then mult = 1 + (mult - 1) * SingleHarvestMutationBonusScale
        -- FastFlag "Game.Selling.SingleHarvestMutationBonusScale" mac dinh 0.15 -> Rainbow x30 tren cay
        -- single chi con ~x5.35. Khong quy doi thi webhook bao loi CAO GAP MAY LAN so voi thuc te.
        if type(mm) == "number" and mm > 1 and Runtime.IsSingleHarvestSeed(cropName) then
            mm = 1 + (mm - 1) * (tonumber(CFG.AutoCollect and CFG.AutoCollect.SingleHarvestMutationBonusScale) or 0.15)
        end
...
-- ============================================================
-- TOC DO NHAT DROP THUONG (qua rung trong workspace.DroppedItems) - chong bao "nhat cham qua".
-- DO CHI PHI THUC TE 1 ITEM o ban cu (moi item = 1 lan di chuyen + 1 lan claim):
--   TweenMoveTo  : dur = khoang_cach / TweenSpeed(22)  -> cach 20 studs = ~0.9s, va CHO TWEEN CHAY HET
--   EndMovement  : 0.2s   |  TeleportWait: 0.05s
--   DropClaimPreWait 0.05s + DropClaimNoPromptWait 0.35s (item khong prompt phai cho het moi thoat)
--   => ~1.5s/item. MaxPerCycle=200 -> ~5 PHUT moi chu ky. Do la nguyen nhan cham.
-- 2 don sua o day (deu co cong tat, mac dinh BAT):
--   1) ClusterClaim  : dung 1 cho -> touch + blast MOI drop trong ban kinh -> 1 lan di chuyen nhat CA CUM.
--                      Qua rung tu cay trong plot nam chum nhau nen an rat manh.
--   2) ApproachDrop  : da dung trong tam thi KHONG di chuyen nua; va cho phep tat tween RIENG cho drop
--                      thuong (DropUseTween=false -> set CFrame thang). Seed event VAN tween nhu cu vi
--                      seed quy hon, can di muot chong server keo.
-- ============================================================

-- DI CHUYEN toi 1 drop thuong. Tra true neu da o vi tri claim duoc.
Runtime.ApproachDrop = function(pos, c)
    local root = getRootPart()
    if not (root and typeof(pos) == "Vector3") then return false end
    -- DA TRONG TAM -> khoi di chuyen (tiet kiem ca PickRouteButton lan TeleportWait moi item).
    local skip = math.max(tonumber(c and c.DropSkipMoveRadius) or 0, 0)
    if skip > 0 and (root.Position - pos).Magnitude <= skip then
        return true
    end
    if c and c.DropUseTween == false then
        local target = Runtime.GetNearTargetPos(pos, c, root.Position)
        if not target then return false end
        pcall(function()
    ...
    return teleportNearPosition(pos, c)
end

-- GOM CUM: dang dung tai cho -> TOUCH moi drop khac trong ban kinh + BLAST prompt 1 lan.
-- Item nao claim duoc se bien mat khoi workspace -> vong for chinh gap lai o buoc sau se bo qua NGAY
-- (check item.Parent) nen khong ton them thoi gian. Blast dung isDangerBlastPrompt san co -> KHONG
-- ban nham prompt mua/gift/GrowPrompt (ton Robux).
Runtime.ClusterTouchDrops = function(items, c)
    if not (c and c.ClusterClaim ~= false) then return 0 end
    local root = getRootPart()
    if not root then return 0 end
    local center = root.Position
    local radius = math.max(tonumber(c.ClusterRadius) or 20, 1)
    local touched = 0
    for _, e in ipairs(items) do
        if e.Source == "Drop" and e.Item and e.Item.Parent and typeof(e.Pos) == "Vector3"
            and (center - e.Pos).Magnitude <= radius then
            if Runtime.TryTouchClaim(e.Item) then touched = touched + 1 end
        end
    end
    if c.ClusterBlastPrompts ~= false then
        -- throttle: blast duyet ca PromptSet nen dung ban moi item khi cum day (ton CPU vo ich).
        local now = os.clock()
        local gap = math.max(tonumber(c.ClusterBlastInterval) or 0.15, 0)
        if (now - (tonumber(Runtime.LastClusterBlast) or 0)) >= gap then
            Runtime.LastClusterBlast = now
            blastNearbyPrompts(center, radius)
        end
    end
    return touched
end
        ...
        if item and item.Parent then
            if entry.Source == "SeedSpawn" then
                ...
                if not Runtime.IsLiveWorkspaceItem(item) then
                    ...
                    Runtime.TryTouchClaim(item)
                    Runtime.ApproachDrop(entry.Pos, c)   -- trong tam -> khong di chuyen; DropUseTween=false -> CFrame thang
                    Runtime.ClusterTouchDrops(items, c)  -- nhat CA CUM quanh cho dang dung, khong 1-item-1-lan-di
                end
...
function Runtime.BuildTierPreset(tier, total)
    ...
    elseif Runtime.WorldId == "FallHarvest" then
        -- MAP 2 (FallHarvest): cay cung vai tro nhung ten la "Maple <ten>" (XAC NHAN SeedData.lua,
        -- Worlds={"FallHarvest"}, RestockShop=true, gia y het ban Main). Dung ten Main o day = mua/trong
        -- HUT (stock luon 0) -> phai tach nhanh rieng.
        if tier == 1 then
            fixed = { ["Maple Carrot"] = 5, ["Maple Strawberry"] = 10, ["Maple Blueberry"] = 10, ["Maple Corn"] = 20 }
            primary = "Maple Tomato"
        elseif tier == 2 then
            fixed = { ["Maple Carrot"] = 5, ["Maple Tulip"] = 5, ["Maple Bamboo"] = 10, ["Maple Apple"] = 2, ["Maple Tomato"] = 42 }
            primary = "Maple Corn"
        else
            fixed = { ["Maple Mushroom"] = 10, ["Maple Grape"] = 10, ["Maple Coconut"] = 10, ["Maple Mango"] = 5, ["Maple Dragon Fruit"] = 5 }
            primary = "Maple Banana"
        end
    elseif tier == 1 then
    ...
    Runtime.LastTierPrimary = primary
    local used = 0
...
function Runtime.doAutoTier()
    ...
    actionLog("AutoTier", "TIER" .. tostring(newTier),
        ("money=%s total=%s primary=%s world=%s"):format(tostring(money), tostring(total),
            tostring(Runtime.LastTierPrimary), tostring(Runtime.WorldId)))
end
...
Runtime.BootStartup = function()
    ...
    -- ["Go To Event"]=true: dang o map 1 -> tu goi remote qua map event de farm (mac dinh TAT).
    Runtime.SafeBoot("GoToEvent", Runtime.SetupGoToEvent)
...
log(("Khởi động. World=%s, %s hiện tại: %s"):format(
    tostring(Runtime.WorldId or "?"), Runtime.CurrencyStatName(), tostring(getSheckles())))
