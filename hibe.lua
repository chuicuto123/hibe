...
-- ===== ANTI TELEPORT / ANTI REJOIN (chong yeu cau): KHOA acc trong PlaceId hien tai =====
-- Chan MOI loi goi TeleportService tu phia CLIENT (script khac / hub ngoai / chinh script nay)
-- + chan LocalPlayer:Kick() (thu doan ep rejoin). Xem Runtime.SetupAntiTeleport.
CFG.AntiTeleport = CFG.AntiTeleport or {}
setDefault(CFG.AntiTeleport, "Enabled", true)
setDefault(CFG.AntiTeleport, "BlockSamePlace", true)  -- true = chan CA server-hop/rejoin cung place
setDefault(CFG.AntiTeleport, "BlockKick", true)       -- chan Player:Kick() tu client
setDefault(CFG.AntiTeleport, "LogEvery", true)        -- in log moi lan chan (co traceback)
...
-- ============================================================
-- ANTI TELEPORT / ANTI REJOIN (chong yeu cau: "khong cho no hook teleport toi ra khoi placeid nay")
-- KHOA acc trong PlaceId hien tai bang cach chan MOI duong teleport tu phia CLIENT:
--   (1) hook __namecall  -> bat kieu goi pho bien nhat: TeleportService:Teleport(...)
--       (moi bien the: Teleport / TeleportAsync / TeleportToPlaceInstance / TeleportToPrivateServer /
--        TeleportPartyAsync / TeleportToSpawnByName) + Player:Kick() (thu doan ep rejoin).
--   (2) hookfunction TRUC TIEP tung method -> bit ca kieu goi "luu ham ra bien roi goi"
--       (local f = TS.Teleport; f(TS, ...)) - kieu nay __namecall KHONG bat duoc.
-- BlockSamePlace=true -> chan CA teleport cung PlaceId (= server-hop / rejoin) -> anti-rejoin.
--
-- >>> GIOI HAN PHAI BIET (khong the lam khac): teleport do SERVER goi (TeleportService chay o server)
--     thi CLIENT KHONG CHAN DUOC - day la co che cua Roblox, khong phai thieu sot cua script.
--     Luc do se KHONG co dong "CHAN" nao trong log ma acc van bay -> ket luan: server chu dong day di.
-- ============================================================
Runtime.AntiTeleportBlocked = 0
Runtime.AntiTeleportShouldBlock = function(targetPlaceId)
    local c = CFG.AntiTeleport
    if not (c and c.Enabled ~= false) then return false end
    local here = game.PlaceId
    if targetPlaceId == nil then return true end                  -- khong ro dich -> chan cho chac
    if targetPlaceId ~= here then return true end                 -- sang place KHAC -> chan
    return c.BlockSamePlace ~= false                              -- cung place = rejoin/server-hop
end
Runtime.AntiTeleportNote = function(what, targetPlaceId)
    Runtime.AntiTeleportBlocked = (Runtime.AntiTeleportBlocked or 0) + 1
    State.LastAntiTeleport = os.date("%H:%M:%S") .. " chan " .. tostring(what)
    local c = CFG.AntiTeleport
    if c and c.LogEvery ~= false then
        local tb = "-"
        pcall(function() tb = debug.traceback("", 3) end)
        actionLog("AntiTeleport", "BLOCKED", ("%s -> place %s (lan thu %d)"):format(
            tostring(what), tostring(targetPlaceId), Runtime.AntiTeleportBlocked))
        logw("[AntiTeleport] CHAN", what, "-> place", tostring(targetPlaceId), "\n--- TRACEBACK ---\n" .. tostring(tb))
    end
end
Runtime.SetupAntiTeleport = function()
    local c = CFG.AntiTeleport
    if not (c and c.Enabled ~= false) or Runtime.AntiTeleportReady then return end
    Runtime.AntiTeleportReady = true
    local TP_METHODS = {
        Teleport = true, TeleportAsync = true, TeleportToPlaceInstance = true,
        TeleportToPrivateServer = true, TeleportPartyAsync = true, TeleportToSpawnByName = true,
    }

    -- (1) __namecall: duong goi pho bien nhat.
    local hookedNamecall = false
    if type(hookmetamethod) == "function" and type(getnamecallmethod) == "function" then
        hookedNamecall = pcall(function()
            local old
            old = hookmetamethod(game, "__namecall", function(self, ...)
                local method = getnamecallmethod()
                if TP_METHODS[method] then
                    local okC, cls = pcall(function() return self.ClassName end)
                    if okC and cls == "TeleportService" then
                        local target = tonumber((...))
                        if Runtime.AntiTeleportShouldBlock(target) then
                            Runtime.AntiTeleportNote("TeleportService:" .. method, target)
                            return nil
                        end
                    end
                elseif method == "Kick" and CFG.AntiTeleport.BlockKick ~= false then
                    -- Kick tu CLIENT = ep dut ket noi -> rejoin. Chi chan khi kick CHINH MINH.
                    if self == LocalPlayer then
                        Runtime.AntiTeleportNote("LocalPlayer:Kick", game.PlaceId)
                        return nil
                    end
                end
                return old(self, ...)
    ...
    -- (2) hookfunction tung method: bit kieu "luu ham ra bien roi goi" (__namecall khong thay).
    local hookedFn = 0
    if type(hookfunction) == "function" then
        local okTS, TS = pcall(function() return game:GetService("TeleportService") end)
        if okTS and TS then
            for method in pairs(TP_METHODS) do
                pcall(function()
                    local orig = TS[method]
                    if type(orig) ~= "function" then return end
                    local okH = pcall(function()
                        hookfunction(orig, function(_, ...)
                            local target = tonumber((...))
                            if Runtime.AntiTeleportShouldBlock(target) then
                                Runtime.AntiTeleportNote("fn TeleportService." .. method, target)
                                return nil
                            end
                            return orig(_, ...)
                        end)
                    end)
                    if okH then hookedFn = hookedFn + 1 end
                end)
            end
        end
    end

    actionLog("AntiTeleport", "ON", ("namecall=%s fn=%d | khoa trong place %s%s"):format(
        tostring(hookedNamecall), hookedFn, tostring(game.PlaceId),
        (c.BlockSamePlace ~= false) and " (chan ca rejoin cung place)" or ""))
    if not hookedNamecall and hookedFn == 0 then
        logw("[AntiTeleport] Executor KHONG cho hook -> KHONG chan duoc teleport tu client.")
    end
    -- Bao ro gioi han 1 lan de chong khong hieu nham la "chan duoc het moi truong hop".
    logw("[AntiTeleport] Luu y: teleport do SERVER goi thi client khong the chan (co che Roblox).")
end
...
-- ANTI TELEPORT dat SOM NHAT co the (truoc ca do hoa/GUI): hook cang som thi cang chan duoc
-- script khac goi teleport o giai doan boot (hub ngoai thuong rejoin/server-hop ngay luc vao).
Runtime.SafeBoot("AntiTeleport", Runtime.SetupAntiTeleport)
